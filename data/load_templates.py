...
class ParseResult(Generic[TTemplateOrDict]):
    def __init__(self, file_path: str = "", errors: list[str] = [], templates: TTemplateOrDict = None):
        self.file_path = file_path
        self.errors = errors
        ...
        if "range" in list_data:
                ...
                result.templates[list_name] = ValueListTemplate(
                    ...
                    values=[
                        ...
                        in range(
                            ...
                            round((int(list_data["range"]["to"]) + 1 - int(list_data["range"]["from"])) * .33)
                        )]
                )
            ...
            try:
                result.templates[list_name] = ValueListTemplate(
                    ...
                    id=list_name,
                    values=[
                        item if isinstance(item, str) else (item["out"] if "out" in item else item["in"])
            ...
            result.errors.append(f"Unknown list type for {list_name} in {file_path}: {json.dumps(list_data)}")
    return result

def parse_expansion_templates(file_path:str, data: dict) -> ParseResult[dict[str, ExpansionRuleTemplate]]:
...
def parse_intent_templates(file_path:str, data: dict) -> (ParseResult[dict[str, IntentTemplate]], int):
    sentence_count = 0
    ...
    for intent_name, intent_data in data.get("intents", {}).items():
        ...
        for item in intent_data.get("data", []):
            try:
                ...
                result.errors.append(f"Error parsing sentences for intent {intent_name} in {file_path}: {e}")
                traceback.print_exc()
...
def parse_yaml_file(file_path: str) -> ParseResult[Templates]:
    ...
    result.templates.errors = lists_result.errors + expansion_rules_result.errors + intents_result.errors
    return result, sentence_count
    ...
    for root, _, files in walk(directory):
        for file in files:
            if file.endswith(".yaml") or file.endswith(".yml"):
                ...
                duplicate_expansion_keys = all_templates.expansion_rules.keys() & result.templates.expansion_rules.keys()
                ...
                all_templates.lists.update({k: v for k, v in result.templates.lists.items() if k not in duplicate_list_keys})
                all_templates.expansion_rules.update({k: v for k, v in result.templates.expansion_rules.items() if k not in duplicate_expansion_keys})
                for intent, intent_data in result.templates.intents.items():
                    if intent not in all_templates.intents:
                        all_templates.intents[intent] = IntentTemplate(file_path="", intent=intent, sentences=[])
...
def _expand_cached(expr, rules_id, lists_id):
    ...
    return tuple(
        s for s in
        (p.strip() for p in _combine(tokens))
        if s
    )
...
def generate_sentences_parallel(
    ...
    *,
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as pool:
        parts = pool.map(
            lambda b: expand(b, rules, prepped),
            branches
        )
